...
Use these prompts when `codex-agent` is asked to review changes rather than implement them.

Prefer `codex exec review` when local git state is the review target. Use `codex exec` only when you need a fully custom prompt contract.
...
2. evidence from code/tests/docs
3. likely impact
</structured_output_contract>
