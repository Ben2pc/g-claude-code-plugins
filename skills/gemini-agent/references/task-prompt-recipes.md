...
Pair this recipe with `--model pro` for the heavy structural pass; switch to `--model flash` for follow-up polish iterations. Use `--approval-mode auto_edit` so iteration on small CSS or layout tweaks does not stall on confirmation prompts.
