...
Use these prompts when `gemini-agent` is asked to review changes rather than implement them.

Pair them with `--approval-mode plan` unless the task explicitly requires write access. `plan` mode keeps Gemini read-only, which is the right default for PR review, diagnosis, and recon.
